#!/bin/bash
# MISP 2.5 installation for Ubuntu 22.04 LTS

# This guide is meant to be a simply installation of MISP on a pristine Ubuntu 22.04 LTS server.
...
function os_version_check() {
    # Check if we're on Ubuntu 22.04 as expected:
    UBUNTU_VERSION=$(sh -c '. /etc/os-release && echo $VERSION_ID')
    if [[ "$UBUNTU_VERSION" != "22.04" ]]; then
        print_error "This installer expects Ubuntu 22.04 LTS. Use INSTALL.ubuntu2404.sh on 24.04 or upgrade the OS first."
        exit 1
    fi
}
...
echo -e "${BLUE}██║ ╚═╝ ██║${NC}██║███████║██║     "
echo -e "${BLUE}╚═╝     ╚═╝${NC}╚═╝╚══════╝╚═╝     "
echo -e "v2.5 Setup on Ubuntu 22.04 LTS"
...
print_status "Adding ondrej/php PPA (PHP 8.2 for Ubuntu 22.04)..."
install_packages software-properties-common
add-apt-repository -y ppa:ondrej/php &>>$logfile
error_check "ondrej/php PPA setup"
apt-get update &>>$logfile
error_check "apt update after ondrej/php PPA"
